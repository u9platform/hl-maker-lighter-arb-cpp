...
double hl_anchored_lighter_price(const SpreadSnapshot& snapshot, Direction direction, double spread_bps) {
    const double spread_ratio = spread_bps / 10000.0;
    if (direction == Direction::ShortLighterLongHl) {
        const double hl_buy_price = snapshot.hl.ask;
        if (hl_buy_price <= 0.0) {
            return 0.0;
        }
        return hl_buy_price * (1.0 + spread_ratio);
    }

    const double hl_sell_price = snapshot.hl.bid;
    if (hl_sell_price <= 0.0) {
        return 0.0;
    }
    return hl_sell_price * (1.0 - spread_ratio);
}

double aggressive_hl_price(const SpreadSnapshot& snapshot, Direction direction) {
    constexpr double kTakerSlippageBps = 10.0;
    if (direction == Direction::ShortLighterLongHl) {
        return snapshot.hl.ask * (1.0 + (kTakerSlippageBps / 10000.0));
    }
    return snapshot.hl.bid * (1.0 - (kTakerSlippageBps / 10000.0));
}
...
LighterMakerHlHedger::LighterMakerHlHedger(StrategyConfig config) : config_(config) {
    if (config_.spread_bps <= 0.0) {
        throw std::invalid_argument("spread_bps must be positive");
    }
    if (config_.cancel_band_bps < 0.0) {
...
StrategyState LighterMakerHlHedger::state() const noexcept {
    return state_;
}

const StrategyConfig& LighterMakerHlHedger::config() const noexcept {
    return config_;
}

const std::optional<PendingMakerOrder>& LighterMakerHlHedger::pending_maker() const noexcept {
    return pending_maker_;
}

const std::optional<OpenHedgePosition>& LighterMakerHlHedger::open_position() const noexcept {
    return open_position_;
}

Action LighterMakerHlHedger::on_market_snapshot(const SpreadSnapshot& snapshot, std::int64_t now_ms) {
    if (!snapshot.valid(config_.max_quote_age_ms)) {
        return {};
    }
    ...
    if (gate_.armed) {
        ...
        if (live_direction != gate_.direction || abs_spread < gate_cancel) {
            gate_.armed = false;
            if (state_ == StrategyState::Idle || state_ == StrategyState::CancelledPendingLighterConfirm) {
                last_disarm_ms_ = now_ms;
            }
        }
    } else if (abs_spread >= config_.spread_bps && can_arm(now_ms)) {
        gate_.armed = true;
        gate_.direction = live_direction;
        gate_.entry_spread_bps = config_.spread_bps;
    }
    ...
    if (state_ == StrategyState::Idle) {
        ...
        pending_maker_ = build_maker_order(snapshot, gate_);
        state_ = StrategyState::PendingLighterMaker;

        Action action;
        action.type = ActionType::PlaceLighterMaker;
        action.reason = "spread reached entry threshold";
        action.maker_order = pending_maker_;
        return action;
    }

    if (state_ == StrategyState::PendingLighterMaker && pending_maker_.has_value()) {
        const double cancel_thresh = cancel_threshold_bps(pending_maker_->entry_spread_bps);
        if (!gate_.armed || live_direction != pending_maker_->direction || abs_spread < cancel_thresh) {
            Action action;
            action.type = ActionType::CancelLighterMaker;
            action.reason = "spread mean-reverted below cancel threshold";
            action.maker_order = pending_maker_;
            state_ = StrategyState::CancelledPendingLighterConfirm;
            disarm(now_ms);
            return action;
        }
    }
...
Action LighterMakerHlHedger::on_lighter_maker_fill(double fill_price, double fill_size_base, const SpreadSnapshot& snapshot) {
    if (!pending_maker_.has_value()) {
        return {};
    }
    ...
    open_position_ = OpenHedgePosition {
        ...
        .size_base = fill_size_base,
        .hl_fill_price = 0.0,
        .lighter_fill_price = fill_price,
    };
    state_ = StrategyState::LighterFilledPendingHlHedge;

    Action action;
    action.type = ActionType::SendHlTakerHedge;
    action.reason = "lighter maker filled";
    action.hl_hedge_intent = build_hl_hedge(snapshot);

    pending_maker_.reset();
    return action;
}

void LighterMakerHlHedger::on_hl_hedge_fill(double fill_price) {
    if (!open_position_.has_value()) {
        return;
    }
    open_position_->hl_fill_price = fill_price;
    state_ = StrategyState::Open;
}

Action LighterMakerHlHedger::on_hl_hedge_reject() {
    if (!open_position_.has_value()) {
        return {};
    }
    state_ = StrategyState::UnwindingLighter;

    Action action;
    action.type = ActionType::UnwindLighterPosition;
    action.reason = "hl taker hedge failed after lighter maker fill";
    return action;
}

void LighterMakerHlHedger::reset() {
    gate_ = {};
    pending_maker_.reset();
    open_position_.reset();
    state_ = StrategyState::Idle;
...
bool LighterMakerHlHedger::can_arm(std::int64_t now_ms) const noexcept {
    return (now_ms - last_disarm_ms_) >= config_.min_rearm_ms;
}

double LighterMakerHlHedger::cancel_threshold_bps(double entry_spread_bps) const noexcept {
    const double threshold = entry_spread_bps - config_.cancel_band_bps;
    return threshold > 0.0 ? threshold : 0.0;
}

Direction LighterMakerHlHedger::direction_for_spread(double cross_spread_bps) const noexcept {
    if (cross_spread_bps >= 0.0) {
        return Direction::ShortLighterLongHl;
    }
    return Direction::LongLighterShortHl;
...
PendingMakerOrder LighterMakerHlHedger::build_maker_order(const SpreadSnapshot& snapshot, const SignalGate& gate) const {
    const Direction direction = gate.direction;
    const double avg_mid = (snapshot.lighter.mid() + snapshot.hl.mid()) / 2.0;
    const double size_base = avg_mid > 0.0 ? config_.pair_size_usd / avg_mid : 0.0;
    const double target_price = hl_anchored_lighter_price(snapshot, direction, gate.entry_spread_bps);

    if (direction == Direction::ShortLighterLongHl) {
        return PendingMakerOrder {
            .direction = direction,
            .is_buy = false,
            .price = std::max(target_price, snapshot.lighter.ask),
            .size_base = size_base,
            .trigger_spread_bps = snapshot.cross_spread_bps,
            .entry_spread_bps = gate.entry_spread_bps,
        };
    ...
    return PendingMakerOrder {
        .direction = direction,
        .is_buy = true,
        .price = std::min(target_price, snapshot.lighter.bid),
        .size_base = size_base,
        .trigger_spread_bps = snapshot.cross_spread_bps,
        .entry_spread_bps = gate.entry_spread_bps,
    };
...
HlHedgeIntent LighterMakerHlHedger::build_hl_hedge(const SpreadSnapshot& snapshot) const {
    if (!open_position_.has_value()) {
        return {};
    }
    const bool is_buy = open_position_->direction == Direction::ShortLighterLongHl;
    return HlHedgeIntent {
        .is_buy = is_buy,
        .limit_price = aggressive_hl_price(snapshot, open_position_->direction),
        .size_base = open_position_->size_base,
    };
}

void LighterMakerHlHedger::disarm(std::int64_t now_ms) noexcept {
    gate_.armed = false;
    last_disarm_ms_ = now_ms;
}
